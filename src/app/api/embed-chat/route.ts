...
${user.id !== 'anonymous' ? '- Ini adalah user terdaftar, berikan layanan yang lebih personalized' : '- Ini adalah user tamu, berikan layanan standar'}` : ''
    ...
    return `Anda adalah asisten AI dari ${widgetConfig.businessName || '7connect'} yang profesional, ramah, dan berpengetahuan luas.${userContext}
    ...
    **Guidelines:**
    ...
    - Berikan informasi yang akurat tentang ${widgetConfig.businessName || '7connect'}
...
${user.id !== 'anonymous' ? '- Ini adalah user terdaftar, berikan layanan yang lebih personalized' : '- Ini adalah user tamu, berikan layanan standar'}` : ''
  ...
  const systemPrompt = `Anda adalah ${persona.name}, asisten AI profesional dari ${widgetConfig.businessName || '7connect'} yang berdedikasi untuk membantu pengguna.${userContext}
...
**Tugas Utama:**
Bantu pengguna seputar layanan ${widgetConfig.businessName || '7connect'}:
1. **AI Chatbot**: Layanan customer service 24/7 dengan AI
2. **WhatsApp Business API**: Integrasi WhatsApp untuk bisnis
3. **WordPress Plugin**: Plugin chat untuk website WordPress
4. **Embed Chat**: Widget chat yang dapat disematkan di website
...
- Bersikap sabar, empatik, dan profesional
- Berikan informasi kontak ${widgetConfig.businessName || '7connect'} jika diperlukan
  ...
  try {
    ...
    const response = await fetch(`${OPENROUTER_BASE_URL}/chat/completions`, {
      ...
      headers: {
        ...
        'X-Title': process.env.NEXT_PUBLIC_APP_NAME || '7connect AI Chatbot'
      },
...
async function generateAIResponse(message: string, widgetConfig: any, persona: any, user?: any): Promise<string> {
  try {
    ...
    if (USE_OLLAMA) {
      ...
      return `Terima kasih atas pesan Anda. Saya dari ${widgetConfig.businessName || '7connect'} siap membantu Anda dengan informasi tentang layanan kami. Mohon maaf, layanan AI sedang dalam konfigurasi. Silakan hubungi tim support kami untuk bantuan lebih lanjut.`
    }
    ...
    return `Maaf, terjadi kesalahan pada sistem AI. Silakan coba lagi nanti atau hubungi tim support ${widgetConfig.businessName || '7connect'}.`
  }
}
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    message: '7connect Embed Chat API',
    version: '1.0.0',
    endpoints: {
