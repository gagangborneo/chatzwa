...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  ...
  if (!post) {
    return {
      title: 'Artikel Tidak Ditemukan - 7connect',
      description: 'Artikel yang Anda cari tidak ditemukan.',
    }
  }

  const title = post.metaTitle || post.title
  const description = post.metaDescription || post.excerpt || `Baca artikel ${post.title} di blog 7connect`
  const keywords = post.metaKeywords || `${post.category}, ${post.tags}, AI chatbot, 7connect, blog Indonesia`
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: 'article',
      url: `https://7connect.id/blog/${post.slug}`,
      publishedTime: post.publishedAt?.toISOString(),
      modifiedTime: post.updatedAt.toISOString(),
      authors: [post.author?.name || '7connect Team'],
      section: post.category,
      tags: post.tags?.split(',').map(tag => tag.trim()) || [],
      images: [
        {
          url: post.ogImage || 'https://7connect.id/og-image-default.jpg',
          width: 1200,
          height: 630,
          alt: post.title,
        },
    ...
    twitter: {
      ...
      title,
      description,
      images: [post.ogImage || 'https://7connect.id/og-image-default.jpg'],
    },
    alternates: {
      canonical: `https://7connect.id/blog/${post.slug}`,
    },
  }
}
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="text-center md:text-left">
                    <p className="text-sm text-gray-600 mb-2">
                      Ditulis oleh <span className="font-semibold text-gray-900">{post.author?.name || '7connect Team'}</span>
                    </p>
